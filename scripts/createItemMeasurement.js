...
module.exports = async (callback) => {
    ...
    try {
        ...
        //const tx = await itemOwner.createItem(web3.utils.fromUtf8(itemName), web3.utils.fromUtf8(itemSkill), itemTokenName, itemTokenSymbol, totalSupply, {from: itemOwnerAddress});
        //console.log(tx);
        return callback();
            ...
            for(b in batches){
                ...
                for(var k=0; k<batch; k++) {
                    try {
                        ...
                        promises.push( itemOwner.createItem(web3.utils.fromUtf8(itemName), web3.utils.fromUtf8(itemSkill), itemTokenName, itemTokenSymbol, totalSupply, {from: itemOwnerAddress}) )                 
                    } catch (error) {
                        console.log(error);
                ...
                fs.writeFileSync("scripts/measurements/testnet/"+network+"/transactions/createItem/sample"+samples+"/"+output+"_"+"batch"+batch+network+".json", json,'utf8');    
            }

            //const json = JSON.stringify(obj, null, 4);
            //fs.writeFileSync("scripts/measurements/"+network+"/transactions/createItem/"+output+"_"+"sample"+s+".json", json,'utf8');
