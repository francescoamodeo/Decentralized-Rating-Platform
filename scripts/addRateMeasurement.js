            ...
            for(b in batches){
                ...
                for(k; k < batch; k++) {
                    try {
                        ...
                        promises.push( user.addRate(itemList[k], score, {from: userAddress}) );                        
                    } catch (error) {
                ...
                fs.writeFileSync("scripts/measurements/testnet/"+network+"/transactions/addRate/sample"+samples+"/"+output+"_"+"batch"+batch+network+".json", json,'utf8');
            }

            //const json = JSON.stringify(obj, null, 4);
            //fs.writeFileSync("scripts/measurements/"+network+"/transactions/addRate/"+output+"_"+"sample"+s+".json", json,'utf8');
